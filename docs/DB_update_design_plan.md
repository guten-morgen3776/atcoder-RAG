...
## 不明点に対する回答（実装前に必ず参照して実装に反映させてください）

1. **実行サマリ行の有無:** 追記する。差分が0件のときでも「実行日時・処理件数0」のサマリ行をログに出力し、cronが正常に動作しているかの死活監視（Heartbeat）として機能するようにしてください。

2. **auto_update の CLI 引数:** 引数なしでよい。個人利用の自動化スクリプトなので、パス類は config.py や .env などの固定値・デフォルト値を参照するシンプルな設計にしてください。

3. **問題の並び順:** 「コンテスト番号昇順・問題インデックス昇順（A, B, C...）」で明示的にソートしてからループ処理に入ってください。途中でエラー終了した際の状況把握を容易にするためです。