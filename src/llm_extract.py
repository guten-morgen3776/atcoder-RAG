...
"""
    ...
    for attempt in range(3):
        try:
            ...
            is_server_overload = "503" in err_str or "UNAVAILABLE" in err_str

            if is_rate_limit or is_server_overload:
                if is_rate_limit:
                    wait = 60
                    label = "Rate limit 429/RESOURCE_EXHAUSTED"
                else:
                    wait = 10 * (2 ** attempt)  # 10s, 20s, 40s
                    label = "Server overload 503/UNAVAILABLE"
                print(f"\n[{label}] Waiting {wait}s before retry (attempt {attempt + 1}/3)...")
                time.sleep(wait)
                continue
